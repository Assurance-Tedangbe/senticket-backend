...
public class DebitHistoryController {
    ...
    @GetMapping(value = "/{debitHistoryId}", produces = "application/json")
    public ResponseEntity<DebitHistoryDTO> getDebitHistoryById(@PathVariable Long debitHistoryId) {

        log.info("Fetched DebitHistory with ID: {}", debitHistoryId);

        DebitHistoryDTO debitHistoryDTO = debitHistoryService
                .readDebitHistoryByDebitHistoryId(debitHistoryId);


        return new ResponseEntity<>(debitHistoryDTO, HttpStatus.OK);
    }
}
